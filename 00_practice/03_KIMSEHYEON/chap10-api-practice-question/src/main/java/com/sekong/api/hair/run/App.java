package com.sekong.api.hair.run;

import com.sekong.api.hair.model.dto.User;
...
public class App {
    public static void main(String[] args) {
        ...
        for(int i = 1; i < str.length ; i++){
            String[] mem = str[i].split(",",-1);
            users[i-1] = new User();
            users[i-1].setId(Integer.valueOf(mem[0]));
            users[i-1].setFirstname(mem[1]);
            users[i-1].setLastname(mem[2]);
            users[i-1].setEmail(mem[3]);
            users[i-1].setGender(mem[4]);

            if (!mem[5].isBlank()) {
                users[i-1].setBirthday(LocalDate.parse(mem[5], DateTimeFormatter.ofPattern("yyyy/MM/dd")));
            } else {
                users[i-1].setBirthday(null);
            }
            users[i-1].setReservationDate(LocalDate.parse(mem[6], DateTimeFormatter.ofPattern("yyyy-MM-dd")));
            users[i-1].setReservationTime(LocalTime.parse(mem[7], DateTimeFormatter.ofPattern("H:m")));
            users[i-1].setCreatedAt(LocalDateTime.parse(mem[8]));
            System.out.println(users[i-1].toString());
        }
