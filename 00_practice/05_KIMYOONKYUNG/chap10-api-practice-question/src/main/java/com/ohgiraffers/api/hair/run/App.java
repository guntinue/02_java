...
public class App {
    ...
    /*예약 데이터를 담을 DTO 클래스입니다. 날짜/시간 필드는 을 사용하며,
    출력 형식에 맞춘 메서드를 오버라이드했습니다.java.timetoString()*/
        ...
        String data = """
                public class App {
                    public static void main(String[] args) {
                        ...
                        ""\";
     // 1. 데이터를 줄 단위로 분리하고, 헤더를 제외한 실제 데이터만 추출
        String[] lines = data.trim().split("\n");
        // 헤더(첫 번째 줄)를 제외하고 데이터를 파싱합니다.
        List<String> dataLines = Arrays.asList(lines).subList(1, lines.length);

        // 2. CSV 데이터를 User 객체 리스트로 파싱합니다.
        List<User> userList = parseCsvData(dataLines);

        // 3. User 객체 배열로 변환
        User[] users = userList.toArray(new User[0]);

        // 4. 결과를 출력
        System.out.println("--- Parsed User Reservation Data (Total: " + users.length + " entries) ---");
        for (User user : users) {
    ...
    /**
     * CSV 형식의 문자열 리스트를 파싱하여 User 객체 리스트로 변환합니다.
     * 데이터 파싱과 관련된 모든 로직과 예외 처리를 담당합니다.
     *
     * @param dataLines CSV 데이터 라인(헤더 제외) 리스트
     * @return 파싱된 User 객체 리스트
     */
    private static List<User> parseCsvData(List<String> dataLines) {
        List<User> userList = new ArrayList<>();

        for (String line : dataLines) {
            // CSV 라인을 쉼표로 분할합니다. -1을 사용하여 모든 필드를 보존합니다.
            ...
            try {
                ...
                // 5. birthday 필드는 빈 문자열일 수 있으므로 null 처리
                LocalDate birthday = fields[5].isEmpty() ? null : LocalDate.parse(fields[5], BIRTHDAY_FORMATTER);

                // reservation_date는 yyyy-MM-dd 형식 (LocalDate의 기본 ISO 포맷)
                LocalDate reservationDate = LocalDate.parse(fields[6]);

                // reservation_time은 H:mm 또는 HH:mm 형식
                // 참고: 'H:mm' 포맷은 한 자리 시간(예: 9)도 처리하며, 'LocalTime.parse()'도 기본적으로 유연합니다.
                // 그러나 데이터에 '9:14'와 같은 형식이 섞여 있으므로, 유연한 'H:mm' 포맷터를 사용합니다.
                LocalTime reservationTime = LocalTime.parse(fields[7], RESERVATION_TIME_FORMATTER);

                // created_at 필드는 LocalDateTime.parse()의 유연한 ISO 포맷 파싱을 사용합니다.
                LocalDateTime createdAt = LocalDateTime.parse(fields[8]);

                // 6. User 객체 생성 및 리스트에 추가
                userList.add(new User(
                        id, firstName, lastName, email, gender, birthday,
                        reservationDate, reservationTime, createdAt
                ));

            } catch (NumberFormatException e) {
                System.err.println("[오류] ID 또는 숫자가 필요한 필드 파싱 중 오류가 발생했습니다: " + line);
                // 오류가 발생한 라인은 건너뛰고 다음 라인을 처리합니다.
            } catch (DateTimeParseException e) {
                System.err.println("[오류] 날짜/시간 파싱 중 오류가 발생했습니다: " + line + ", 에러: " + e.getMessage());
                // 오류가 발생한 라인은 건너뛰고 다음 라인을 처리합니다.
            } catch (Exception e) {
                System.err.println("[오류] 알 수 없는 오류가 발생했습니다: " + line + ", 에러: " + e.getMessage());
                // 오류가 발생한 라인은 건너뛰고 다음 라인을 처리합니다.
            }
        }
