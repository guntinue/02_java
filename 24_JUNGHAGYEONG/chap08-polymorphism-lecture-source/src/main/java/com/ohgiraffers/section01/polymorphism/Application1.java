...
public class Application1 {
    ...
    public static void main(String[] args) {
        ...
        /*
         * Rabbit과 Tiger는 Animal클래스를 상속 받음
         *
         * Rabbit은 Rabbitㅏ입이기도 하면서 Animal타입이기도 함
         * Tiger는 Tiger타입이기도 하면서 Animal타입이기도 함
         * */
...
//        Tiger tiger1 = new Animal();
        ...
        /*
         * 객체별로 고유한 기능을 동작시키기 위해서는 레퍼런스 변수를 형변환하여 Rabbit과 Tiger로 변경해야 메소드 호출 가능
         * class type casting : 클래스 형변환
         * ==> 타입 형변환시 실제 인스턴스의 타입이 일치하지 않는 경우 ClassCastException이 발생 가능함
         * */
...
//        ((Tiger) animal1).bite();
        ...
        if (animal1 instanceof Rabbit) { // true
            ((Rabbit) animal1).jump();
        }

        if (animal1 instanceof Tiger) { // false
            ((Tiger) animal1).bite();
        }

        /*
         * 클래스 형변환은 up-casting과 down-casting으로 구분 가능
         * up-casting : 상위 타입으로 형변환
         * down-casting : 하위 타입으로 형변환
         * 또한 작성여부에 따라 명시적과 묵시적 두가지로 구분 됨
         * */
        ...
        Rabbit rabbit1 = (Rabbit) animal3;    // Animal ->  Rabbit 다운캐스팅
                                              // 명시적 형변환
//        Rabbit rabbit2 = animal3;             // 다운캐스팅 묵시적 형변환 안됨
