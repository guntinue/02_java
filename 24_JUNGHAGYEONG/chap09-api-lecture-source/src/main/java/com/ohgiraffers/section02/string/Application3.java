...
public class Application3 {
    ...
    public static void main(String[] args) {
         ...
         *           정규표현식을 이용하기 때문에 속도가 느리다는 단점을 가짐
         *           정규표현식은 자바에서는 다루지 않고 자바스크립트에서 다룰 예정
         * StringTokenizer : 문자열의 모든 문자들을 구분자로 하여 문자열을 분리함
         *                   정형화된 문자열 패턴을 분리할 대 사용하기 좋음 (공백 문자열 무시)
         *                   split()보다 속도면에서는 더 빠름
         *                   구분자를 생략하는 경우 공백이 기본 구분자임
         * */
        // 사번/이름/주소/부서
        ...
        while (st1.hasMoreTokens()) { // hasMoreTokens() : 현재 꺼낼 토큰이 있는지 없는지 체크
            System.out.println("st1 : "+ st1.nextToken()); // nextToken() : 현재 값을 빼서 반납
        }
