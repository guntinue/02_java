...
//상속
//인터페이스를 상속 시 implements 키워드 대신 extends를 사용한다.
...
// ==> &로 여러 타입을 동시에 가지는 경우에만 타입변수에 맞는 타입으로 여긴다.(둘 중 하나가 아니다)
// public class RabbitFarm<T extends Rabbit & Animal> {

// 클래스를 여러개는 &사용이 불가하며 ,(콤마) 사용 시 컴파일 에러는 발생하지 않지만
// 작성한 클래스를 또 다른 타입변수로 본다. -> 하나의 클래스만 상속받아 구현할 수 있다.
