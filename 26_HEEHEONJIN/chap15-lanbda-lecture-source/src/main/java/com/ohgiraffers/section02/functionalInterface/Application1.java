...
public class Application1 {
    public static void main(String[] args) {
        ...
        /*Consummer<T>#accept(T t) : 객체 T를 받아 소비*/
        Consumer<String> consumer = (String s) -> System.out.println(s + "이(가) 입력됨");
        consumer.accept("helloworld!");

        /*BiConsumer<T, U>@accept(T t, U u):void ==> 객체 T, U 를 받아 소비한다. */
        BiConsumer<String, LocalTime> biConsumer = (String str1, LocalTime time) -> 
                System.out.println(str1 + "이(가) " + time + "이 입력됨");
        biConsumer.accept("홀길동", LocalTime.now());
...
//        /*ObjIntConsumer#accpt(T t, int value):void ==> 객체 T와 int값을 받아 소비한다.*/
//        ObjIntConsumer<java.util.Random> objIntConsumer = (Random random, bound) -> System.out.println("0부터 " + bound + "전까지의 난수 발생:"
...
//        objIntConsumer.accept(new Random(), 10);
        ...
        ObjLongConsumer<LocalDate> objLongConsumer =
                (date, days) -> System.out.println(date + "의" + days + "일 후의 날짜는"
                + date.plusDays(days));
        ...
        List<String> names = Arrays.asList("홍길동", "심사임당", "인순신");
