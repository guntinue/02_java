...
public class Application3 {
    public static void main(String[] args) {
        ...
        while(st3.hasMoreTokens()){ //hasMoreTokens() : 현재 꺼낼 토큰이 있는지 없는지 체크
            System.out.println("st3 : " + st3.nextToken()); // nextToken() : 현재값을 뺴서 반납
        }

        System.out.println("=====");
        while(st3.hasMoreTokens()){ // 다시하면 안 나온다. 전에 이미 빼내서. 이게 토크나이저 특징
            System.out.println("st3 : " + st3.nextToken());
        }

        String colorStr = "red*orange#blue/yellow green"; // 이럴때 정규표현식 사용
        //String[] colors = colorStr.split(""); // "#/" 로 구분되는지
        ...
        while(colorStrTokenizer.hasMoreTokens()){
            System.out.println("color : " + colorStrTokenizer.nextToken());
        }
    }
}
